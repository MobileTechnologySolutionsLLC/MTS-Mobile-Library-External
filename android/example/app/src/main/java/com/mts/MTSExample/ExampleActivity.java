...
public class ExampleActivity extends AppCompatActivity implements ActivityCompat.OnRequestPermissionsResultCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        AppCenter.start(getApplication(), "c54d2a5f-e37f-4cb2-9e53-513b21eae674",
    ...
    public void onResume() {
        ...
        if (null != mtsService) {
            ...
            updateInterface();
            mtsService.checkForAccessories("onResume");
        }
    }
    ...
    private void bluetoothEnabler() {
        mtsService.bluetoothEnabler(this);
    }
    ...
    private void assignPlayerIdForAccessory() {
        mtsService.assignCardDataForWriteOnNextUSBConnect(lastCardData());
    }
    ...
    private final BroadcastReceiver mtsServiceUpdateReceiver = new BroadcastReceiver()
    {
        ...
        public void onReceive(Context context, Intent intent)
        {
            ...
            if (mtsService.AccessoryConnectionStateChanged.equals(action)) {
                if (mtsService.accessoryConnectionState == MTSService.AccessoryConnectionState.connected) {
                    Log.v(TAG, "USB Accessory connected.  No action/implementation required here. If cardData has been previously assigned to the MTSService, the cardData will automatically be written to the USB accessory.");
                }
            }
            else if (MTSService.BluetoothConnectionStateChanged.equals(action)){
                if (mtsService.bluetoothConnectionState == connected) {
                    playConnectedSound();
                    requestTerminalKind();
//                    writePlayerIdToBluetooth();
                    lastWriteAtTextView.setText("Last write: none since connect.");
                }
                else if (mtsService.bluetoothConnectionState == scanning
            ...
            else if (MTSService.DidWriteCardDataToAccessory.equals(action)){
                Boolean success = intent.getExtras().getBoolean("writeOutcome");
                if (success) {
                    // The wired card reader accessory does not provide a playerId read response;
                    // at this point we know the bytes have been written to the stream without
                    // error.  Look at the card reader bezel for confirmation that the write worked.
                    Log.v(TAG, "DidWriteCardDataToAccessory.");
                } else {
                    Log.v(TAG, "Failed to write cardData to accessory.");
                }
            }
            updateInterface();
        }
    ...
    private static IntentFilter mtsServiceUpdateIntentFilter() {
        ...
        intentFilter.addAction(MTSService.AccessoryConnectionStateChanged);
        ...
        intentFilter.addAction(MTSService.DidWriteCardDataToAccessory);
        return intentFilter;
    }
