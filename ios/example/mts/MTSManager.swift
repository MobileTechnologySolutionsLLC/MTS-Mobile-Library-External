...
enum MTSError: Error {
    ...
    case accessoryConnectionNotReady(description: String)
}

extension EAAccessory {
    var usesSupportedProtocol: Bool {
        get {
            let accessorySupportedProtocols = self.protocolStrings
            let bundleProtocols = Bundle.main.object(forInfoDictionaryKey: "UISupportedExternalAccessoryProtocols")
            guard let appSupportedProtocols = bundleProtocols as? [String] else {
                NSLog("\(#function) failed to load bundled protocols.")
                return false
            }
            let accSet = Set(accessorySupportedProtocols)
            let appSet = Set(appSupportedProtocols)
            let matchCount = accSet.intersection(appSet).count
            return matchCount > 0
        }
    }
}
...
public class MTSManager: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    public var accessoryConnectionState = AccessoryConnectionState.notReady
    ...
    private var dataToWriteToAccessory = Data()
    ...
    public func writeCardDataToAccessory(cardData: String) throws {
        
        let truncatedString = cardData.prefix(cardDataCharacterCountMax)
        ...
        try writeDataToAccessory(data: terminatedData)
    }
...
private extension MTSManager {
    ...
    func invokeAccessoryConnectionStateChangedDelegate() {
        DispatchQueue.main.async {
            self.delegate.invokeDelegates({ delegate in
                delegate.accessoryConnectionStateChanged()
            })
        }
    }
    ...
    func invokeReceivedTerminalId(terminalId: String) {
        DispatchQueue.main.async {
            self.delegate.invokeDelegates({ delegate in
                delegate.receivedTerminalId(terminalId: terminalId)
            })
        }
    }
    ...
    func invokeDidWriteCardDataToAccessory(error: Error?) {
        DispatchQueue.main.async {
            self.delegate.invokeDelegates({ delegate in
                delegate.didWriteCardDataToAccessory(error: error)
            })
        }
    }
...
extension MTSManager: EAAccessoryDelegate, StreamDelegate {
    
    func setupForAccessory() {
        connectedAccessories = EAAccessoryManager.shared().connectedAccessories
        EAAccessoryManager.shared().registerForLocalNotifications()
        addObservers()
        connectAfterSetupIfNeeded()
    }
    
    private func connectAfterSetupIfNeeded() {
        let connectedAccessory = connectedAccessories.filter {$0.usesSupportedProtocol}.first
        guard nil != connectedAccessory else {
            return
        }
    ...
    private func addObservers() {
        ...
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(self.accessoryDidDisconnectViaNotification),
    ...
    @objc private func accessoryDidConnect(notification: Notification) {
        guard let connectedAccessory = notification.userInfo?[EAAccessoryKey] as? EAAccessory else {
            logIfEnabled("\(#function) failed to identify the connected accessory")
            return
        }
        guard connectedAccessory.usesSupportedProtocol else {
            logIfEnabled("\(#function) accessory does not use a supported protocol.")
            return
        }
        logIfEnabled("\(#function)")
        connectedAccessories.append(connectedAccessory)
        accessory = connectedAccessory
        openSession()
        // N.B. Don't invokeAccessoryConnectionStateChangedDelegate here, wait until
        // the .openCompleted stream event signifies we are ready to handle writes.
    }
    
    @objc private func accessoryDidDisconnectViaNotification() {
        closeSession()
        accessoryConnectionState = .notReady
        invokeAccessoryConnectionStateChangedDelegate()
    }
    
    public func accessoryDidDisconnect(_ accessory: EAAccessory) {
        accessoryDidDisconnectViaNotification()
    }
    
    private func matchingProtocolString(_ accessory: EAAccessory) -> String? {
        
        let accessorySupportedProtocols = accessory.protocolStrings
        
        let bundleProtocols = Bundle.main.object(forInfoDictionaryKey: "UISupportedExternalAccessoryProtocols")
        
        guard let appSupportedProtocols = bundleProtocols as? [String] else {
            logIfEnabled("Please update this app's Info.plist for key \"Supported external accessory protocols\".")
            return nil
        }
        
        let accSet = Set(accessorySupportedProtocols)
        let appSet = Set(appSupportedProtocols)
        let matchingProtocols = Array(accSet.intersection(appSet))
        
        guard let matchingProtocol = matchingProtocols.first else {
            logIfEnabled("\(#function) failed to find matching protocols.")
            return nil
        }
    ...
    private func supportedAccessory() -> EAAccessory? {
        for connectedAccessory in connectedAccessories {
            if connectedAccessory.usesSupportedProtocol {
    ...
    private func openSession() {
        ...
        guard let accessory = accessory else {
            logIfEnabled("\(#function) failed because no accessory was assigned.")
            return
        }
        
        guard let protocolString = matchingProtocolString(accessory) else {
            logIfEnabled("\(#function) failed because not matching protocols were found.")
            return
        }
        
        accessory.delegate = self
        
        logIfEnabled("\(#function) Attempting session start for accessory: \(accessory.name) with protocol: \(protocolString)")
        guard let session = EASession(accessory: accessory, forProtocol: protocolString) else {
            logIfEnabled("\(#function) failed to open session.")
            return
        }
        
        // N.B.
        // Optional chaining of session?.inputStream?.etc seemed like it should work, but it does not.
        // It is necessary to unwrap then assign to a retained property as below.
        if let i = session.inputStream, let o = session.outputStream {
            i.delegate = self
            i.schedule(in: .current, forMode: RunLoop.Mode.default)
            i.open()
            inputStream  = i
            
            o.delegate = self
            o.schedule(in: .current, forMode: RunLoop.Mode.default)
            o.open()
            outputStream = o
        }
        else {
            logIfEnabled("\(#function) failed due to nil stream issue.")
        }
    }
    ...
    private func closeSession() {
        ...
        outputStream = nil
        accessory = nil
        logIfEnabled("\(#function) complete.")
    }
    
    private func writeDataToAccessory(data: Data) throws {
        dataToWriteToAccessory.append(data)
        writeData()
    }
    
    private func writeData() {
        guard let stream = outputStream else {
            logIfEnabled("\(#function) failed because outputStream was nil.")
            return
        }
        
        while(stream.hasSpaceAvailable &&
            dataToWriteToAccessory.count > 0
        ) {
            let bytesWrittenCount = stream.write(dataToWriteToAccessory)
            
            if -1 == bytesWrittenCount {
                logIfEnabled("\(#function) failed with write error.")
                break
            }
            else if (bytesWrittenCount > 0) {
                let range = 0..<bytesWrittenCount
                dataToWriteToAccessory.replaceSubrange(range, with: Data())
                logIfEnabled("\(#function) bytesWrittenCount: \(bytesWrittenCount) dataToWriteToAccessory.count: \(dataToWriteToAccessory.count)")
                if 0 == dataToWriteToAccessory.count {
                    invokeDidWriteCardDataToAccessory(error: nil)
                }
            }
        }
    }
    ...
    public func stream(_ aStream: Stream, handle eventCode: Stream.Event) {
        ...
        case .openCompleted:
            ...
            logIfEnabled("\(#function) .openCompleted")
            accessoryConnectionState = .connected
            invokeAccessoryConnectionStateChangedDelegate()
        case .hasBytesAvailable:
            logIfEnabled("\(#function) .hasBytesAvailable")
        break // The MTS card reader accessory never sends values to the app.
        case .hasSpaceAvailable:
            logIfEnabled("\(#function) .hasSpaceAvailable")
            writeData()
        case .errorOccurred:
            logIfEnabled("\(#function) .errorOccurred")
            let error = MTSError.writeFailure(description: "Stream error occurred.")
            invokeDidWriteCardDataToAccessory(error: error)
        case .endEncountered:
            logIfEnabled("\(#function) .endEncountered")
            aStream.close()
            aStream.remove(from: .current, forMode: RunLoop.Mode.default)
            aStream.delegate = nil
            if aStream == outputStream {
                outputStream = nil
                dataToWriteToAccessory = Data()
            }
            else if aStream == inputStream {
                inputStream = nil
            }
        default:
            break // Why is default needed for exhaustive switch: Stream.Event is an OptionSet, not Swift Enum.
        }
        logIfEnabled("\(#function) inputStream.streamStatus: \(String(describing: aStream.streamStatus.rawValue)) outputStream?.streamStatus: \(String(describing: aStream.streamStatus.rawValue))")
    }
